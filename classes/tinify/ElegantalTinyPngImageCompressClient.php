...
class ElegantalTinyPngImageCompressClient
{
    ...
    public function __construct($key, $app_identifier = null, $proxy = null)
    {
        $curl = curl_version();

        if (!($curl["features"] & CURL_VERSION_SSL)) {
            throw new ElegantalTinyPngImageCompressClientException("Your curl version does not support secure connections");
        }

        if ($curl["version_number"] < 0x071201) {
            $version = $curl["version"];
            throw new ElegantalTinyPngImageCompressClientException("Your curl version ${version} is outdated; please upgrade to 7.18.1 or higher");
        }
        ...
        if ($proxy) {
            ...
            if (isset($parts["host"])) {
                ...
                throw new ElegantalTinyPngImageCompressConnectionException("Invalid proxy");
            }
    ...
    public function request($method, $url, $body = null)
    {
        ...
        for ($retries = self::RETRY_COUNT; $retries >= 0; $retries--) {
            ...
            if ($request === false || $request === null) {
                throw new ElegantalTinyPngImageCompressConnectionException("Error while connecting: curl extension is not functional or disabled.");
            }

            curl_setopt_array($request, $this->options);

            $url = Tools::strtolower(Tools::substr($url, 0, 6)) == "https:" ? $url : self::API_ENDPOINT . $url;
